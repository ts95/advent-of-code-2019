...
class Day3 {
    ...
    func getWirePaths() -> (Path, Path) {
        let lines = getInput().split(separator: "\n")
        let path1 = lines[0].split(separator: ",").map(Steps.init)
        let path2 = lines[1].split(separator: ",").map(Steps.init)
        return (path1, path2)
    }
    ...
    struct Point: Hashable {
        ...
        func rectilinearDistance(to point: Point) -> Int {
            abs(x - point.x) + abs(y - point.y)
        }

        mutating func moveOneStep(direction: Direction) {
            switch direction {
            case .up:
                self.y -= 1
            case .left:
    ...
    typealias Path = [Steps]

    struct Steps {
        let direction: Direction
        let count: Int

        init<SP: StringProtocol>(_ string: SP) {
            direction = Direction(rawValue: string.first!)!
            count = Int(string.dropFirst())!
        }

        func visitsAfterApplyingSteps(to point: inout Point) -> Set<Point> {
        ...
        func visitedPointStepsAfterApplyingSteps(to point: inout Point, visitedPointSteps: inout [Point : Int]) -> Set<Point> {
            var visitedPoints = Set<Point>()
            for i in (1...count) {
                point.moveOneStep(direction: direction)
                visitedPointSteps[point] = i
                visitedPoints.insert(point)
            }
            return visitedPoints
        }

        /// Passing visitedPoints by reference without mutating it (because it's huge and it would copied by default)
        func crossingWiresAfterApplyingSteps(to point: inout Point, visitedPoints: inout Set<Point>) -> Set<Point> {
            var crossingWirePoints = Set<Point>()
            for _ in (0..<count) {
                point.moveOneStep(direction: direction)
                if visitedPoints.contains(point) {
                    crossingWirePoints.insert(point)
                }
            }
            return crossingWirePoints
        }
    }
...
extension Day3: Challenge {

    func getAnswerPartOne() -> Any? {
        let (path1, path2) = getWirePaths()

        var wireTipPoint1 = Point.zero
        var visitedPoints = Set<Point>([Point.zero])

        for steps in path1 {
            visitedPoints.formUnion(steps.visitsAfterApplyingSteps(to: &wireTipPoint1))
        }

        var wireTipPoint2 = Point.zero
        var crossingWirePoints = Set<Point>()

        for steps in path2 {
            let crossingWires = steps.crossingWiresAfterApplyingSteps(to: &wireTipPoint2,
                                                                      visitedPoints: &visitedPoints)
            crossingWirePoints.formUnion(crossingWires)
        }

        let closestPointToCentralPort = crossingWirePoints.min(by: {
            Point.zero.rectilinearDistance(to: $0) < Point.zero.rectilinearDistance(to: $1)
        })!

        return Point.zero.rectilinearDistance(to: closestPointToCentralPort)
    }
    ...
    func getAnswerPartTwo() -> Any? {
        ...
        for steps in path1 {
            for _ in (0..<steps.count) {
                wireTipPoint1.moveOneStep(direction: steps.direction)
                path1TotalSteps += 1
                path1VisitedPointSteps[wireTipPoint1] = path1TotalSteps
            }
        }

        var wireTipPoint2 = Point.zero
        var path2TotalSteps = 0
        var path2VisitedPointSteps = [Point : Int]()
        var crossingWirePoints = Set<Point>()

        for steps in path2 {
            for _ in (0..<steps.count) {
                wireTipPoint2.moveOneStep(direction: steps.direction)
                path2TotalSteps += 1
                path2VisitedPointSteps[wireTipPoint2] = path2TotalSteps
                if path1VisitedPointSteps.keys.contains(wireTipPoint2) {
                    crossingWirePoints.insert(wireTipPoint2)
                }
            }
        }

        return crossingWirePoints.map {
            path1VisitedPointSteps[$0]! + path2VisitedPointSteps[$0]!
        }.min()!
    }
